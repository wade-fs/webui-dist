...
function PostStatus(){
  ...
  mac=$1; shift
  status=$1; shift
  tip=192.168.2.$(shuf -i 2-253 -n 1)
  [ -z "${status}" ] && status="ACTIVE"
  echo curl -s http://${ip}:8088/api-q8c/status-force/${mac}/${tip}/${status}
  curl -s http://${ip}:8088/api-q8c/status-force/${mac}/${tip}/${status} -o - | grep OK >/dev/null && exit 0
